...
//        userRegistration("login1", "pass1", "nick1");
    ...
    public boolean getAllRegistredUsers(Vector v) throws SQLException{
        if (v !=null) {
            PreparedStatement psFindAllNicks;
            psFindAllNicks= connection.prepareStatement("SELECT nick FROM users");
            ResultSet rs = psFindAllNicks.executeQuery();
            System.out.println("getAllRegistredUsers:");
            String s;
            while (rs.next()) {
                s = rs.getString("nick");
                System.out.println(s);
                v.add(new ConnectionList(s, false));
            }
            return (!v.isEmpty());
        }
        return false;
